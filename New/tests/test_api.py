...
def test_health_config_and_ports():
    ...
    assert len(bootstrap.json()["shipProfiles"]) == 7
...
def test_multi_route_validation_and_success():
    ...
    body = {"ports": ["INBOM", "LKCMB"], "use_weather": False, "use_alert_zones": False, "vessel": {"ship_type": "bulk_carrier", "fuel_onboard_t": 9000, "fuel_capacity_t": 10000, "displacement_ex_fuel_t": 38000, "reference_displacement_t": 45000, "engine_mcr_kw": 18000}}
    response = client.post("/api/multi-route", json=body)
    assert response.status_code == 200, response.text
    assert response.json()["summary"]["voyage_legs"] == 1
    assert response.json()["ship_type"] == "bulk_carrier"
    assert response.json()["ship_profile"] == "Bulk carrier"
