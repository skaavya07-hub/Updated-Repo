...
def segment_performance(distance_nm: float, speed_kn: float, current_component_kn: float, wind_head_ms: float, wave_head_m: float, fuel_t: float, v: VesselParameters) -> SegmentPerformance | None:
    ...
    resistance = 1 + max(0, wind_head_ms) * 0.009 + max(0, wave_head_m) * 0.055
