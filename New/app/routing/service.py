...
def calculate_leg(graph, origin, destination, departure, fuel_start, vessel, priorities, use_weather, use_alerts, avoidance, leg_index=0, prefer_alternate=True):
    ...
    def best_edge(u, v, distance, fuel=fuel_start, when=departure):
        ...
        if use_weather and (conditions.wave_m > vessel.max_wave_height_m or conditions.wind_ms > vessel.max_wind_speed_ms):
        ...
        options = []
        for speed in (vessel.service_speed_kn * x for x in (0.68, 0.78, 0.88, 0.96, 1.03)):
            perf = segment_performance(distance, speed, current, head_wind, head_wave, fuel, vessel)
            if perf:
                fuel_norm = perf.fuel_t / max(1, distance * 0.08)
                time_norm = perf.hours / max(1, distance / vessel.service_speed_kn)
                weather_risk = conditions.wave_m / vessel.max_wave_height_m * 0.55 + conditions.wind_ms / vessel.max_wind_speed_ms * 0.45
                cost = wf * fuel_norm + wt * time_norm + ws * (weather_risk + alert_penalty * 4)
