...
def calculate_multi(request):
    ...
    cells = 0; max_wave = max_wind = 0.0
    for index, (origin, destination) in enumerate(zip(request.ports, request.ports[1:])):
        leg, evaluated, hits, weather = calculate_leg(GRAPH, origin, destination, departure, fuel, request.vessel, request.priorities, request.use_weather, request.use_alert_zones, request.alert_avoidance, index)
    ...
    return RouteResponse(combined_route=combined, legs=legs, summary=summary, warnings=warnings, environmental_source="Deterministic time-indexed climatological fallback (Open-Meteo adapter ready)", alert_zone_exposure=sorted(exposure))
